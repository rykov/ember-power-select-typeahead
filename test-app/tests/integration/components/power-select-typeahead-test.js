...
module('Integration | Component | <PowerSelectTypeahead>', function (hooks) {
  ...
  test('It can select options when options are strings', async function (assert) {
    ...
    await render(hbs`
      <PowerSelectTypeahead @options={{this.numbers}} @selected={{this.selected}} @onChange={{fn (mut this.selected)}} as |number|>
        {{number}}
  ...
  test('It can select options when options are objects', async function (assert) {
    ...
    await render(hbs`
      <PowerSelectTypeahead @options={{this.countries}} @selected={{this.selected}} @onChange={{fn (mut this.selected)}} @searchField="name" @extra={{hash labelPath="name"}} as |country|>
        {{country.name}}
      </PowerSelectTypeahead>
    `);
    assert.notOk(
